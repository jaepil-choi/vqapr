...
RI = "00_register_instruments"
ORIGINALS = len({f for f in TRACES[RI]["files_after"] if not f.startswith(".vqapr/")})

HEADER = {
    "title": "vqapr 시나리오 디버거 · 등록 정리",
    "storage_key": "vqapr-stepper-0161",
    "eyebrow": "vqapr 0.16.1 · records 280-284 · redesign/registration-cleanup · 2026-09-14 · sys.setprofile 트레이스",
    "h1": "vqapr 시나리오 디버거 — 등록 정리 후",
    "lede": (
        "샘플 프로젝트(10종목 · 735거래일)에서 여덟 장면을 실제로 돌리고 한 호출씩 따라갑니다. "
        "프레임마다 어느 명령인지, 한두 문장, 접힌 곳에 함수와 호출 번호가 있습니다. "
        "0.16.0 판 이후 바뀐 곳은 <b>[280]</b>~<b>[284]</b>입니다. "
        "숫자는 모두 트레이스와 run 기록에서 읽었습니다(<code>experiments/exp_282_the_scenario_trace_registration_cleanup/</code>)."
    ),
    "facts": [
        {"k": "흐름", "v": "명단 · 송장 → 입고 문 → 장부 → 점검 → 라인 → 일지", "s": "YAML을 검사해 장부에 적고, run은 점검 뒤 루프를 돌아 기록을 남긴다"},
        {"k": "[284] 명단은 따로", "v": "<code>instruments.yaml</code>", "s": "데이터는 명단 없이 등록된다. 명단은 주문할 수 있는 종목만 정한다"},
        ...
        {"k": "명령 17개", "v": f"호출 {nc(SH)} ~ {nc(SL)}", "s": f"명단 {nc(RI)} · 등록 {nc(R)} · stop-loss run {nc(SL)}"},
    ],
    "fix": (
        f"<strong>ms</strong>는 프로파일러 아래 값이라 실제보다 큽니다. 같은 트레이스 안에서만 비교하십시오. "
        f"첫 parquet 스캔은 {ms(R, 181)} ms, 두 번째는 {ms(R, 362)} ms입니다(식은 디스크)."
    ),
    "glossary": [],
}
...
SRCMAP_LEDE = "프레임이 선 파일을 포함한 199개 파일 설명. 0.16.0 판에서 옮기고, 등록 정리가 바꾼 파일의 설명과 줄 수를 0.16.1 트리에 맞췄습니다."

SCENES = [
    {
        "id": "reg", "key": "①", "title": "데이터 등록",
        "sub": f"register instruments.yaml · {nc(RI)} 호출 — register sample.yaml · {el(R)} ms · {nc(R)} 호출",
        "story": "<b>[284]</b> 명단(<code>instruments.yaml</code>)과 송장(<code>sample.yaml</code>)은 따로 냅니다. 명단은 주문할 수 있는 종목을, 송장은 데이터 · 부품 · run을 적습니다. dataset은 입고 문을 지나고, 전부 통과해야 장부를 한 번에 씁니다.",
        "shelf": [RI, R],
        "frames": [
            F(RI, 0, "[284] 명단은 따로 낸다", "명단에는 주문할 수 있는 종목만 적습니다. 데이터는 명단 없이도 등록되고, 데이터의 종목이 모두 명단에 있을 필요도 없습니다.",
              chain(RI, (13, "read_declaration"), (14, "apply"), (36, "_instruments")),
              fn="main()", code=at("src/vqapr/cli/main.py", "def main(", 8),
              disk={".vqapr/": "없음"}),
            F(RI, 39, "종목 명단을 연다", f"종목 명단 파일(<code>instruments_stock.parquet</code>: 종목 코드 · 종류, 10행)을 엽니다. {ms(RI, 39)} ms는 pyarrow를 처음 올리는 비용입니다.",
              chain(RI, (39, "verify_roster"), (43, "build_roster")),
              fn="verify_roster()", code=("def", 8), data="roster"),
            F(RI, 104, "장부가 처음 생긴다", "임시 파일에 쓰고 바꿔 끼우므로 반쯤 쓴 장부는 보이지 않습니다. 명단 포인터도 함께 씁니다.",
              chain(RI, (91, "Transaction.commit"), (97, "Workspace._write"), (104, "write_atomically"), (107, "Workspace._write_roster")),
              fn="write_atomically()", code=at("src/vqapr/_internal/atomic.py", "staged = Path(staged_name)", 10),
              disk={".vqapr/workspace.yaml": "생김 · 명단만", ".vqapr/instruments.json": "생김"}),
            ...
            F(R, 14, "[280] 입고 카트를 연다", "함수 하나가 카트를 열어 모았다가 끝에 한 번 씁니다(예전엔 둘). 하나라도 거절되면 장부는 그대로입니다.",
              chain(R, (16, "Workspace.transaction"), (102, "stage_measured"), (440, "_component"), (880, "Transaction.commit")),
              fn="apply()", code=at(REG, 'for dataset_id, body in section("datasets")', 7)),
            F(R, 102, "[281] 입고 문 — 하나", "dataset은 모두 이 문으로 들어옵니다: 재고, 걸리면 거절, 통과하면 잰 카드를 카트에. run이 만든 출력도 같은 문입니다.",
              chain(R, (102, "stage_measured"), (103, "verify_source"), (249, "raise_if_failed"), (250, "Transaction.register_dataset")),
              fn="stage_measured()", code=at(REG, "def stage_measured(", 15)),
            F(R, 103, "[281] 검수대 — 이름, 그리고 여섯 가지", "카드와 파일의 이름부터 맞춰 보고, 컬럼 → 키 중복 → 기간 → NaN → 체결 가격 → 지문. 파일 내용을 보는 곳은 여기뿐입니다.",
              chain(R, (104, "mismatched_source"), (105, "describe"), (117, "check_schema"), (160, "check_key"), (181, "check_span"), (192, "check_values"), (246, "physical_digest")),
            ...
            F(R, 407, "체결표도 같은 문으로", "거래 가능한 행마다 가격이 양수인지 재서 <code>['close']</code>를 적어 둡니다.",
              chain(R, (297, "stage_measured"), (298, "verify_source"), (407, "check_execution_prices")),
              fn="check_execution_prices()", code=("def", 8), data="execution",
              mem={"sample-execution": "체결 가격 ['close'] · 지문 49e4b4ab…"}),
            F(R, 525, "부품 시운전", "전략 · 거래소 코드를 import해 필요한 메서드가 있는지 봅니다.",
              chain(R, (444, "fingerprint_component"), (525, "conformance"), (697, "conformance")),
              fn="conformance()", code=("def", 8)),
            F(R, 880, "카트를 장부로", "잠금을 잡고 장부를 다시 읽어, 카트의 카드를 한 번 더 합칩니다.",
              chain(R, (880, "Transaction.commit"), (893, "_merge_dataset"), (910, "Workspace._write")),
              fn="Transaction.commit()", code=("def", 10)),
            F(R, 946, "장부를 다시 쓴다", "명단 뒤에 dataset 둘 · 부품 둘 · run을 더해, 이번에도 임시 파일에 쓰고 바꿔 끼웁니다.",
              chain(R, (946, "write_atomically"), (947, "_swap")),
              fn="write_atomically()", code=at("src/vqapr/_internal/atomic.py", "staged = Path(staged_name)", 10), data="ledger_prices",
              disk={".vqapr/workspace.yaml": "고쳐 씀 · + dataset 둘 · 부품 둘 · run"}),
        ],
        "remember": ["명단은 따로 — 주문할 수 있는 종목만.", "dataset은 모두 입고 문 하나로 들어온다.", "장부는 카트에 모아 한 번에, 원자적으로 쓴다."],
    },
    {
    ...
    },
    {
        ...
        "frames": [
            ...
            F(RA, 606, "같은 송장으로 다시 등록", "두 dataset이 입고 문을 다시 지나며 파일을 다시 잽니다.",
              chain(RA, (410, "stage_measured"), (411, "verify_source"), (605, "stage_measured"), (606, "verify_source")),
              fn="verify_source()", code=("def", 8)),
            F(RA, 742, "[281] 잰 값만 교체", "이름 대조는 검수대와 같은 규칙입니다(어긋나면 400). 당신이 쓴 부분이 같으면 기간 · 지문만 바꾸고, 다르면 409로 거절합니다.",
              chain(RA, (742, "_merge_dataset"), (743, "mismatched_source"), (1218, "Workspace._write")),
    ...
    },
    {
        ...
        "frames": [
            ...
            F(FA, 2284, "dataset은 출처를 가리지 않는다", "run이 만든 <code>sample-features</code>도 지문으로 묶습니다.",
              chain(FA, (2284, "_validate_requirement"), (2285, "Workspace.require_verified")),
              fn="_validate_requirement()", code=("def", 8)),
            F(FA, 2345, "공구 봉지", "전략 · 거래소 · 기간을 꺼내기만 합니다.",
              chain(FA, (2345, "RunResources.of"), (2361, "run")),
              fn="RunResources.of()", code=("def", 8)),
            F(FA, 2370, "종목 명단은 매번 새로", "명단은 늘어나므로 얼리지 않습니다(이슈 009).",
              chain(FA, (2370, "registered_roster"), (2377, "verify_roster")),
              fn="registered_roster()", code=("def", 8)),
            F(FA, 2561, "표지 · 팻말 · 진행 메모", "run이 도는 동안 디스크엔 이 셋뿐입니다.",
              chain(FA, (2484, "freeze_run_record"), (2561, "RunRecordWriter.open"), (3112, "write_atomically", ": progress.json")),
              fn="RunRecordWriter.open()", code=at("src/vqapr/record/writer.py", "def _claim(self)", 10),
              data="run_json", disk={".vqapr/runs/sample-factor-run/run.json": "생김", FACTOR_DIR + ".running": "생김", FACTOR_DIR + "progress.json": "생김"}),
            F(FA, 3264, "읽을 범위 — 11 × 10", "run 기간만큼 행렬을 만듭니다.",
              chain(FA, (3264, "_scan_bounds"), (3289, "observation_table"), (3326, "Panel.from_table")),
              fn="_scan_bounds()", code=at("src/vqapr/data/store.py", "for lookback in lookbacks:", 8, before=2)),
            F(FA, 3222, "여섯 종목을 고른다", "하위 3에 −1/6, 상위 3에 +1/6. 사건은 <code>ScheduledEvent</code>, 시각은 <code>call.at</code>.",
              chain(FA, (3114, "RunLoop.handle"), (3222, "SampleFactor.decide"), (3557, "_stamp_intent")),
              fn="SampleFactor.decide()", code=at(DECL + "factor.py", "def decide(self, call):", 10), author=True, data="factor_weight"),
            F(FA, 3817, "행은 메모리에", f"weight 행을 Arrow 버퍼에 쌓을 뿐 파일은 쓰지 않습니다(이 run {count(FA, 'RunRecordWriter.append_chunk')}번).",
              chain(FA, (3815, "RunStateRepository.publish"), (3817, "RunRecordWriter.append_chunk")),
              fn="append_chunk()", code=at("src/vqapr/record/writer.py", "def append_chunk(self, chunk: RecordChunk)", 8)),
            F(FA, 3852, "15:30 체결", "+23 · +8 · +94 / −55 · −19 · −12주. 현금 99,463,501.24.",
              chain(FA, (3844, "RunLoop.handle"), (3852, "ExecutionHandler.fill"), (4326, "AcademicExchange.execute")),
              fn="ExecutionHandler.fill()", code=("def", 8), data="factor_fill",
              mem={"계좌 v1": "롱 3 · 숏 3 · NAV 1억"}),
            F(FA, 4886, "계좌가 스스로 평가", "<code>Account.mark</code>가 보유 × 가격을 계산합니다.",
              chain(FA, (4886, "ValuationHandler.mark"), (4935, "Account.mark")),
              fn="ValuationHandler.mark()", code=("def", 8), data="factor_account"),
            F(FA, 24212, "마감 — 본문, 그다음 도장", "<code>all.parquet</code> 셋(70 · 73 · 60행) → <code>strategy.json</code> → 팻말 치움.",
              chain(FA, (24215, "_write_compact"), (24261, "write_atomically"), (24263, "RunRecordWriter._unlock")),
            ...
            F(FA, 24470, "[281] 비중을 dataset으로", "60행을 <code>materialized/</code>에 쓰고 같은 입고 문을 지나 장부에 올립니다.",
              chain(FA, (24470, "RunOutput.register"), (24514, "stage_measured"), (24645, "Workspace._write")),
    ...
    },
    {
        ...
        "shelf": [SR, SL],
        "frames": [
            F(SL, 6428, "memory 복원", "결정 전마다 memory를 복원합니다. 첫날은 비어 있습니다.",
              chain(SL, (6409, "_visible_callback_state"), (6428, "_restore_callback_state")),
              fn="_restore_callback_state()", code=("def", 8)),
            F(SL, 6545, "행렬 하나로 37일", "38 × 10 행렬 하나를 모든 결정이 나눠 봅니다.",
              chain(SL, (6545, "_scan_bounds"), (9984, "_scan_bounds")),
              fn="_scan_bounds()", code=at("src/vqapr/data/store.py", "for lookback in lookbacks:", 8, before=2)),
            F(SL, 6509, "첫 결정 — 9종목 진입", "종가 9개를 memory에 적고 같은 비중으로 삽니다.",
              chain(SL, (6509, "SampleStopLoss.decide")),
              fn="SampleStopLoss.decide()", code=at(DECL + "stoploss.py", 'if not self.memory.get("entered"):', 8, before=4), author=True),
            F(SL, 7744, "결정 후 저장", "memory를 JSON으로 정리해 계좌와 함께 저장합니다.",
              chain(SL, (7744, "_candidate_callback_state"), (7941, "publish")),
              fn="_candidate_callback_state()", code=("def", 8)),
            F(SL, 9950, "둘째 날 첫 손절", "K000005가 −3%를 넘어 팝니다. 보유: 9 → 8 → 7 → 5 → 4 → 3 → 2 → 1.",
              chain(SL, (9867, "_restore_callback_state"), (9950, "SampleStopLoss.decide")),
              fn="SampleStopLoss.decide()", code=at(DECL + "stoploss.py", "for name, price in list(entry.items()):", 5), author=True, data="stop_weight"),
            F(SL, 58150, "전부 팔 땐 빈 Rebalance", "Hold면 포지션이 남습니다. K000008 −52주, 현금 84,184,068.92.",
              chain(SL, (58150, "SampleStopLoss.decide"), (58590, "fill")),
              fn="SampleStopLoss.decide()", code=at(DECL + "stoploss.py", "held = {name: 1 for name in sorted(entry)}", 7), author=True, data="stop_last"),
            F(SL, 59279, "그 뒤는 Hold", "주문이 없으니 오후엔 평가만 합니다.",
              chain(SL, (59279, "SampleStopLoss.decide"), (59662, "ValuationHandler.mark")),
    ...
    },
    {
        ...
        "shelf": [ER, EN, LD, SH],
        "frames": [
            F(EN, 2615, "alpha도 지문으로", "run이 만든 파일과 원본 파일이 같은 취급을 받습니다.",
              chain(EN, (2615, "_validate_requirement"), (2634, "_validate_requirement")),
              fn="_validate_requirement()", code=("def", 8)),
            F(EN, 3604, "1/9에 alpha의 절반", "01-13 08:00의 비중을 읽어 0.1944 · 0.0278 · 0.1111로 나눕니다.",
              chain(EN, (3604, "SampleEnhancedIndex.decide"), (4425, "Panel.from_table"), (4537, "Panel.from_table")),
              fn="SampleEnhancedIndex.decide()", code=at(DECL + "enhanced.py", "tilted = {", 5), author=True, data=["factor_alpha", "enhanced_weight"]),
            F(EN, 5217, "롱온리 체결", "9종목을 삽니다. 9일 뒤 계좌 v9.",
              chain(EN, (5217, "ExecutionHandler.fill"), (27414, "RunOutput.register")),
    ...
    },
    {
        ...
        "frames": [
            ...
            F(WK, 16, "작업자도 preflight", "check가 거절할 run은 작업자도 거절합니다.",
              chain(WK, (16, "preflight"), (1194, "freeze"), (1643, "RunResources.of")),
              fn="preflight()", code=at("src/vqapr/run/assemble.py", "def run_registered_strategy", 8)),
            F(WK, 2610, "스캔 대신 매핑", f"cube의 지문이 맞으면 파일을 매핑해 읽습니다. 스캔 {count(WK, 'observation_table')}회.",
              chain(WK, (2584, "open_cube"), (2610, "panel_from_cube")),
...
TABLE = {
    "title": "트레이스로 확인한 것",
    "rows": [
        ("[284] 명단은 따로", f"00 {h(RI, 39, 'verify_roster')} · 01의 명단 호출 {count(R, 'verify_roster')}번", "데이터는 명단 없이 등록되고, 명단은 주문할 수 있는 종목만 정한다"),
        ("[281] 입고 문 하나", f"01 {h(R, 102, 'stage_measured')} · {h(R, 297, 'stage_measured')} · 02 {h(B, 387, 'stage_measured')} · 06 {h(DM, 7820, 'stage_measured')} · 08 {h(FA, 24514, 'stage_measured')} · 12 {h(EN, 27458, 'stage_measured')}", "선언 · 거절 · run 출력이 같은 문을 지난다"),
        ("[281] 이름 대조는 규칙 하나", f"01 {h(R, 104, 'mismatched_source')} 검수대 · {h(R, 255, 'mismatched_source')} 카트", "검수대와 장부 합치기가 같은 함수를 묻는다 · 400"),
        ...
        ("기록 순서", f"08: {h(FA, 2484, 'freeze_run_record')} → {h(FA, 2561, 'RunRecordWriter.open')} → {h(FA, 3112, 'write_atomically')} → {h(FA, 24215, '_write_compact')} → {h(FA, 24261, 'write_atomically')} → {h(FA, 24263, '_unlock')} → {h(FA, 24645, 'Workspace._write')}", "도장이 있으면 본문이 있다"),
