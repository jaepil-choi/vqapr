...
to write, and `vqapr register instruments.yaml` registers them. `vq.register_instruments(project_root,
{id: kind})` writes those tables and registers them in one call. The roster does not have to cover
