...
### 2026-09-15의 번호 없는 보고 다섯 — 시나리오 testbed run 5(`0.16.1` wheel, `9c54f211`) — 전부 닫혔다 (campaign `redesign/sections-and-one-register`, records `296`–`299`)
...
| 파일 | 제목 | 상태 | 어디로 가는가 |
|---|---|---|---|
| ~~`report-2026-09-15-no-record-or-public-surface-names-the-vqapr-version-...`~~ | run을 쓴 vqapr 버전을 record·envelope·`__version__`·`--version` 어디도 말하지 않는다 | **닫힘 — record `298`.** strategy·datamodel record의 `package_version`, `vqapr.__version__`, `vqapr --version`; run identity에는 넣지 않는다(오너). "이번 버전에서 바뀐 것" skill 노트는 오너 질문으로 남았다 | 코드 + skill |
| ~~`report-2026-09-15-strategy-report-raises-bare-valueerror-...`~~ | NAV ≤ 0인 record에서 `strategy_report`가 맨 `ValueError`로 여섯 절을 다 잃는다 | **닫힘 — record `297`.** 절마다 한 문(`SectionUndefined`); NAV ≤ 0이면 `performance`·`book`·`intent`만 이유와 함께 빠지고 `attribution`·`compliance`와 `trading`의 금액은 남는다; export가 `report.json`을 쓴다 | 코드 |
| ~~`report-2026-09-15-yaml-re-register-...-omits-the-replaced-fingerprint`~~ | 선언 YAML로 바뀐 전략을 다시 등록하면 `replaced`가 없다(세 인자 형식만 말한다) | **닫힘 — record `296`.** 병합이 두 길 모두에 `replaced`를 답한다; 선언은 `{<id>: {fingerprint}}` | 코드 |
| ~~`report-2026-09-15-batch-envelope-has-no-batch-elapsed-...`~~ | `--jobs` 배치 envelope에 배치 경과 시간이 없고, `timing.total`이 무엇을 덮는지 문서가 없다 | **닫힘 — record `299`.** 여러 run의 envelope에 `elapsed`와(`--jobs`면) `bake`; skill이 `timing.total`의 범위를 말한다 | 코드 + skill |
| ~~`report-2026-09-15-docs-do-not-say-a-decision-right-after-a-fill-...`~~ | 체결 직후의 결정은 직전 목표의 비중을 읽는다 — 어디에도 쓰여 있지 않다 | **닫힘 — commit `ab0a6b81`.** `make-strategy/references/reading-inputs.md`의 "Your own book, right after a fill" | skill |

### 2026-09-14의 번호 없는 보고 하나 — scenario testbed run 4(`0.16.0` wheel) — 접수, 닫힘 (campaign `redesign/strategy-budget`, record `291`)

run 4가 낸 22건 중, budget 캠페인이 부수적으로 닫는 하나만 여기서 접수한다. 나머지는 아직 미분류다.

| 파일 | 제목 | 상태 | 닫은 것 |
|---|---|---|---|
| ~~`report-2026-09-14-sample-strategy-imports-from-private-vqapr-domain-intent`~~ | `vqapr new sample`의 전략이 `Budget`·`PortfolioDirection`을 private `vqapr.domain.intent`에서 import한다 | **닫힘 2026-09-15 — record `291`.** 소스에서 확인(`agent/sample/reversal_5d.py:16`). budget이 전략 클래스의 선언(`vq.Budget`)이 되고 `PortfolioDirection`이 사라지면서 샘플은 `vqapr.public`만 import한다 | 코드 |
